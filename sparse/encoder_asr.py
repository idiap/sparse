...
#
"""
This is where the SNN-based encoder is defined, transforming acoustic
to phonemic features for automatic speech recognition (ASR) tasks.
"""
import torch
...
class ASREncoder(nn.Module):
    ...
    In order to output 25Hz signals that work well with CTC loss for
    subword classification, average pooling is applied to downsample
    the spike trains, after which some (non-spiking) fully connected
    layers transform it into phoneme/token probabilities.
    ...
    cortex_use_lateral_rnn : bool
        ...
        where radius=rnn_portion*layer_size. If False, the nonzero elements of
        the recurrent matrix are randomly chosen based on rnn_portions.
    phoneme_num_layers : int
        Number of non-spiking fully-connected layers after average pooling
        spike trains from the last layer of cortical neurons.
    phoneme_features : int
        Number of features in fully-connected layers.
    phoneme_ctc_rate_hz : int
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Phoneme Avgpooling and FC layers
        self.pool_phoneme = nn.AvgPool1d(
            kernel_size=int(1000 / dt / phoneme_ctc_rate_hz)
        )
        self.linear_phoneme = [
            nn.Linear(cortex_layer_sizes[-1], phoneme_features),
            activation(),
            nn.Dropout(p=dropout),
        ]
        if phoneme_num_layers > 1:
            for _ in range(phoneme_num_layers - 1):
                self.linear_phoneme += [
                    nn.Linear(phoneme_features, phoneme_features),
                    activation(),
                    nn.Dropout(p=dropout),
                ]
        self.linear_phoneme = nn.Sequential(*self.linear_phoneme)
        ...
        print_num_params(self.linear_phoneme, "Linear phoneme")
    ...
    def forward(self, feats):
        ...
        # Encode acoustic features into spikes
        ...
        # Decode spikes into phoneme features
        x = self.pool_phoneme(x.transpose(2, 1)).transpose(2, 1)
        x = self.linear_phoneme(x)
